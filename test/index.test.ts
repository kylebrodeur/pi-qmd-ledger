...
function createMockApi() {
  ...
  const api = {
    ...
    registerCommand: (name: string, opts: any) => commands.push({ name, args: [name, opts] }),
    on: (event: string, handler: any) => events.push({ name: event, args: [event, handler] }),
    getActiveTools: () => [],
...
describe("Config system", () => {
  let tmpDir: string;
  beforeEach(() => { tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "qmd-test-")); });
...
describe("Ledger CRUD", () => {
  let tmpDir: string;
  let ext: any;
  let tools: Map<string, (id: string, params: any, signal: any, onUpdate: any, ctx: any) => Promise<any>>;
  ...
  it("appends to master in autopilot mode", async () => {
    ...
    const result = await append("tc-1", {
      ledger: "master",
      mode: "autopilot",
      entry: { id: "r1", domain: "test", source: "test", fact: "hello", tag: "t1", artifact: "" },
  ...
  it("prevents duplicate in autopilot when dedupField matches", async () => {
    ...
    // first append
    await append("tc-1", {
      ledger: "master", mode: "autopilot",
      entry: { id: "r1", domain: "test", source: "test", fact: "dup-test", tag: "t1", artifact: "" },
    }, undefined, undefined, ctx as any);

    // second append with same fact (dedupField)
    const result = await append("tc-2", {
      ledger: "master", mode: "autopilot",
      entry: { id: "r2", domain: "test", source: "test", fact: "dup-test", tag: "t2", artifact: "" },
    }, undefined, undefined, ctx as any);
  ...
  it("queries by exact filter", async () => {
    ...
    await append("tc-1", { ledger: "master", mode: "autopilot", entry: { id: "r1", domain: "test", source: "s1", fact: "a", tag: "alpha", artifact: "" } }, undefined, undefined, ctx as any);
    await append("tc-2", { ledger: "master", mode: "autopilot", entry: { id: "r2", domain: "test", source: "s2", fact: "b", tag: "beta", artifact: "" } }, undefined, undefined, ctx as any);

    const result = await query("tc-3", { ledger: "master", filters: { tag: "alpha" } }, undefined, undefined, ctx as any);
  ...
  it("describes ledger with counts and samples", async () => {
    ...
    await append("tc-1", { ledger: "master", mode: "autopilot", entry: { id: "r1", domain: "test", source: "s1", fact: "first", tag: "t", artifact: "" } }, undefined, undefined, ctx as any);
    await append("tc-2", { ledger: "master", mode: "autopilot", entry: { id: "r2", domain: "test", source: "s2", fact: "second", tag: "t", artifact: "" } }, undefined, undefined, ctx as any);

    const result = await describe("tc-3", { ledger: "master" }, undefined, undefined, ctx as any);
  ...
  it("exports to csv format", async () => {
    ...
    await append("tc-1", { ledger: "master", mode: "autopilot", entry: { id: "r1", domain: "test", source: "s1", fact: "hello", tag: "t", artifact: "" } }, undefined, undefined, ctx as any);

    const result = await exportTool("tc-2", { ledger: "master", format: "csv" }, undefined, undefined, ctx as any);
    const text = result.content[0].text;
  ...
  it("exports to markdown format", async () => {
    ...
    const result = await exportTool("tc-2", { ledger: "master", format: "markdown" }, undefined, undefined, ctx as any);
    const text = result.content[0].text;
  ...
  it("stats reports correct counts", async () => {
    ...
    await append("tc-1", { ledger: "master", mode: "autopilot", entry: { id: "r1", domain: "test", source: "s1", fact: "f1", tag: "t", artifact: "" } }, undefined, undefined, ctx as any);
  ...
  it("configure_ledger updates config at runtime", async () => {
    ...
    const result = await configure("tc-1", { action: "update", config: { qmd: { defaultLimit: 20 } } }, undefined, undefined, ctx as any);
...
describe("Injector system", () => {
  ...
  it("injects matching entries on before_agent_start", async () => {
    ...
    const cfg = {
      ...
      injectors: [{ name: "test-inj", regex: "draft\\s+(\\S+)", ledger: "master", filterField: "tag" }],
    ...
    fs.writeFileSync(path.join(tmpDir, "ledger", "master.jsonl"), JSON.stringify({ id: "r", domain: "test", source: "s", fact: "injected-fact", tag: "login", artifact: "" }) + "\n");
    ...
    const ctx = createMockContext(tmpDir);
    const result = await handler({ prompt: "draft login", systemPrompt: "base prompt" }, ctx as any);
  ...
  it("returns undefined when no injector matches", async () => {
    ...
    const ctx = createMockContext(tmpDir);
    const result = await handler({ prompt: "hello world", systemPrompt: "base" }, ctx as any);
